...
class CustomConverter(ASRCustomConverter):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        """
        #_, ys = batch[0]
        xs, ys_tgt, ys_src = batch[0]
        #ys_asr = copy.deepcopy(ys)
        xs_pad, ilens, ys_tgt_pad = super().__call__([(xs, ys_tgt)], device)
        #xs_pad, ilens, ys_pad = super().__call__(batch, device)
        _, _, ys_src_pad = super().__call__([(xs, ys_src)], device)
        """
        ...
        return xs_pad, ilens, ys_tgt_pad, ys_src_pad
...
def trans(args):
    ...
    if args.batchsize == 0:
        with torch.no_grad():
            ...
            print("TOTAL_NUM:", total_num, flush=True)
            print("AVERAGE_TIME:", total_time / total_num, flush=True)
        ...
        with torch.no_grad():
            ...
            print("AVERAGE_TIME:", total_time / total_num)
...
def extract_encoder_feat(args):
    ...
    # read json data
    save_json_pth = os.path.join(args.extract_layer_path, args.trans_json.split('/',3)[2])
    if args.trans_json.split('/',3)[2].startswith(  args.trans_json.split('/',3)[2] ):
        check_sp = True
    else:
        check_sp = False

    if not os.path.isfile(os.path.join(save_json_pth, 'data.json')):
        with open(args.trans_json, "rb") as f:
            js = json.load(f)["utts"]
    else:
        with open(os.path.join(save_json_pth, 'data.json'), "rb") as f:
            js = json.load(f)["utts"]
    ...
    assert(args.extract_layer > 0)
    save_pth = os.path.join(save_json_pth, 'seg8_layer{}'.format(args.extract_layer))
    #save_pth = os.path.join(save_json_pth, 'layer_{}'.format(args.extract_layer))
    ...
    if args.batchsize == 0:
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                #if (not check_sp) or name.startswith('sp1.0'):
                    logging.info("(%d/%d) decoding " + name, idx, len(js.keys()))
                    batch = [(name, js[name])]
                    feat = load_inputs_and_targets(batch)[0][0]
                    layer_feat = model.extract_feat(feat, args, train_args.char_list, rnnlm)
                    # save feat
                    np.save(os.path.join(save_pth, name), layer_feat.detach().numpy())
                    js[name]['seg8_feat_layer{}'.format(extract_layer)] = os.path.abspath(os.path.join(save_pth, name+'.npy'))


    with open(os.path.join(save_json_pth, 'data.json'), "wb") as f:

        f.write(
            json.dumps(
                {"utts": js}, indent=4, ensure_ascii=False, sort_keys=True
            ).encode("utf_8")
        )
