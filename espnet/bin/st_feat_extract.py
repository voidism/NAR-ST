...
def get_parser():
    ...
    parser.add_argument("--extract_layer", type=int, default=0, help="Number of GPUs")
    parser.add_argument("--extract_layer_path", type=str, default='./encoder_extract_feat', help="Number of GPUs")
    parser.add_argument(
        "--dtype",
...
def main(args):
    ...
    if args.backend == "pytorch":
        ...
        from espnet.st.pytorch_backend.st import extract_encoder_feat

        if args.dtype != "float32":
            raise NotImplementedError(
                f"`--dtype {args.dtype}` is only available with `--api v2`"
            )
        extract_encoder_feat(args)
    else:
