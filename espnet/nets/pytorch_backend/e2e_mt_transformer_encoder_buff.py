...
class E2E(MTInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        self.to_ctc_linear = torch.nn.Linear(args.adim//2, args.adim)
        self.ctc = CTC(
                odim, args.adim, args.dropout_rate, ctc_type='builtin', reduce=True
            )
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        ## NOTE divide and copy : make encoder output two times longer (time axis)
        batch_size = xs_pad.size(0)
        hs_pad = hs_pad.view(batch_size, -1, self.adim//2)
        hs_pad = self.to_ctc_linear(hs_pad)
        #hs_mask = hs_mask.view(batch_size, -1).sum(1) * 2
        ...
        # 3. compute CTC loss
        hs_len = hs_mask.view(batch_size, -1).sum(1) * 2
        self.loss = self.ctc(
            hs_pad, hs_len, ys_out_pad
            #hs_pad.view(batch_size, -1, self.adim), hs_len, ys_out_pad
                )
        #self.loss = self.criterion(pred_pad, ys_out_pad)

        #self.acc = th_accuracy(
        #    self.ctc.ctc_lo(hs_pad).view(-1, self.odim), ys_out_pad, ignore_label=self.ignore_id
        ...
        if self.training:
            ...
            ys_hat = self.ctc.argmax(hs_pad)
            #ys_hat = pred_pad.argmax(dim=-1)
            self.bleu = self.error_calculator(ys_hat.cpu(), ys_pad.cpu())

        ### check this, use bleu replace acc
        self.acc = self.bleu if (self.bleu != None) else 0.0
    ...
    def translate(self, x, trans_args, char_list=None, rnnlm=None, use_jit=False):
        ...
        h = enc_output
        batch_size = xs_pad.size(0)
        h = h.view(batch_size, -1, self.adim//2)
        h = self.to_ctc_linear(h)
