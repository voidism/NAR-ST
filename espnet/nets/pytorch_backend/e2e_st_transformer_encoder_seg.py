...
class E2E(STInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        assert(args.elayers > args.elayers_seg)
        num_blocks_src = args.elayers_seg
        num_blocks_tgt = args.elayers - args.elayers_seg
        ...
        self.encoder_src = Encoder(
            ...
            num_blocks=num_blocks_src,
            input_layer=args.transformer_input_layer,
            dropout_rate=args.dropout_rate,
            positional_dropout_rate=args.dropout_rate,
            attention_dropout_rate=args.transformer_attn_dropout_rate,
        )
        self.encoder_tgt = Encoder(
            idim=args.adim, ### Check
            attention_dim=args.adim,
            attention_heads=args.aheads,
            linear_units=args.eunits,
            num_blocks=num_blocks_tgt,
            input_layer=None,
            dropout_rate=args.dropout_rate,
            positional_dropout_rate=args.dropout_rate,
            attention_dropout_rate=args.transformer_attn_dropout_rate,
    ...
    def forward(self, xs_pad, ilens, ys_pad, ys_pad_src):
        ...
        hs_pad_src, hs_mask_src = self.encoder_src(xs_pad, src_mask)
        hs_pad_tgt, hs_mask_tgt = self.encoder_tgt(hs_pad_src, hs_mask_src)
        ...
        # linear transform via argmax function
        #hs_pad_ctc = self.ctc.argmax(hs_pad)
        #hs_pad_ctc
        #hs_pad_ctc_mask = ys_out_pad != self.ignore_id
        #numerator = torch.sum(
        #        hs_pad_ctc.masked_select(hs_mask.squeeze(1)) == ys_out_pad.masked_select(hs_pad_ctc_mask)
        #        )
        #denumerator = torch.sum(hs_pad_ctc_mask)
        #self.acc = float(numerator) / float(denumerator)
        ...
        #        )
        batch_size = xs_pad.size(0)
        hs_len_src = hs_mask_src.view(batch_size, -1).sum(1)
        hs_len_tgt = hs_mask_tgt.view(batch_size, -1).sum(1)

        loss_ctc_src = self.ctc_src(
            hs_pad_src.view(batch_size, -1, self.adim), hs_len_src, ys_out_pad_asr
            #hs_pad.view(batch_size, -1, self.adim), hs_len, ys_pad_src
        )
        loss_ctc_tgt = self.ctc(
            hs_pad_tgt.view(batch_size, -1, self.adim), hs_len_tgt, ys_out_pad
            #hs_pad.view(batch_size, -1, self.adim), hs_len, ys_pad_src
        )
        if self.error_calculator is not None:

            ys_hat_src = self.ctc_src.argmax(hs_pad_src.view(batch_size, -1, self.adim)).data
            cer_ctc_src = self.error_calculator(
                ys_hat_src.cpu(), ys_pad_src.cpu(), is_ctc=True
            )

            ys_hat = self.ctc.argmax(hs_pad_tgt.view(batch_size, -1, self.adim)).data
            cer_ctc_tgt = self.error_calculator(
                ys_hat.cpu(), ys_pad.cpu(), is_ctc=True
            )
        # TODO
        # Cant compute acc, use negative cer instead
        self.acc = - cer_ctc if (cer_ctc != None) else 0.0
        self.acc_asr = - cer_ctc_src if (cer_ctc_src != None) else 0.0
        # 5. compute cer/wer
        ...
        alpha = self.mtlalpha
        self.loss = loss_ctc_src * alpha + loss_ctc_tgt * (1 - alpha)
        #self.loss = (
        #    (1 - self.asr_weight - self.mt_weight) * loss_att
        #    + self.asr_weight * (alpha * loss_ctc + (1 - alpha) * loss_asr)
        #    + self.mt_weight * loss_mt
        #)
        loss_asr_data = float(loss_ctc_src)
        #loss_asr_data = float(alpha * loss_ctc_src + (1 - alpha) * loss_asr)
        loss_mt_data = None if self.mt_weight == 0 else float(loss_mt)
        loss_st_data = float(loss_ctc_tgt)
    ...
    def encode(self, x):
        ...
        x, _ = self.encoder_src(x, None)
        enc_output, _ = self.encoder_tgt(x, None)
        return enc_output.squeeze(0)


    def extract_feat(self, x, trans_args, char_list=None, rnnlm=None, use_jit=False):

        self.eval()
        extract_layer = trans_args.extract_layer
        assert(extract_layer > 0)
        x = torch.as_tensor(x).unsqueeze(0)
        # first pass by the conv layer
        out_feat = self.encoder_src.embed(x, None)
        #modulelist = list(self.encoder.encoders.modules())
        # out_feat = tuple(feat, None)
        for l in self.encoder_src.encoders[:extract_layer]:
            out_feat = l(out_feat[0], None)
        # out_feat[0] --> shape = (time_len, 256)
        return out_feat[0].squeeze(0)
    ...
    def translate(self, x, trans_args, char_list=None, rnnlm=None, use_jit=False):
        ...
        for i in six.moves.range(maxlen):
           ...
           #     )[:beam]
            ...
            new_hyp['yseq'] = ctc_decode
            hyps_best_kept = [new_hyp]
            hyps = hyps_best_kept
