...
def load_trained_model(model_path):
    ...
    if hasattr(train_args, "num_buff") and train_args.num_buff > 0:
        if hasattr(model, "encoder"):
            model.encoder.add_buff()
        else:
            model.encoder_src.add_buff()
            model.encoder_tgt.add_buff()
                ...
                if is_lm:
                    ...
                    if partial_state_dict:
                        ...
                        if transfer_verification(
                            ...
                            # check if multitasking encoder
                            if sum([mk.startswith('encoder_src') for mk in main_state_dict.keys()]):
                                seg_layer = args.elayers_seg
                                src_dict = \
                                        get_partial_state_dict(main_state_dict, \
                                        ['encoder_src.embed', 'encoder_src.encoders'])
                                tgt_dict = \
                                        get_partial_state_dict(main_state_dict,\
                                        [ 'encoder_tgt.encoders'])


                                if 'encoder.after_norm.weight' in partial_state_dict.keys():
                                    main_state_dict['encoder_src.after_norm.weight'] = \
                                            partial_state_dict['encoder.after_norm.weight'].clone()
                                    main_state_dict['encoder_src.after_norm.bias'] = \
                                            partial_state_dict['encoder.after_norm.bias'].clone()
                                    main_state_dict['encoder_tgt.after_norm.weight'] = \
                                            partial_state_dict['encoder.after_norm.weight'].clone()
                                    main_state_dict['encoder_tgt.after_norm.bias'] = \
                                            partial_state_dict['encoder.after_norm.bias'].clone()
                                    logging.warning("override src/tgt after_norm")
                                    partial_state_dict.pop('encoder.after_norm.weight')
                                    partial_state_dict.pop('encoder.after_norm.bias')
                                #if 'ctc.ctc_lo.weight' in model_state_dict.keys():
                                #    main_state_dict['ctc_src.ctc_lo.weight'] = \
                                #            model_state_dict['ctc.ctc_lo.weight'].clone()
                                #    main_state_dict['ctc_src.ctc_lo.bias'] = \
                                #            model_state_dict['ctc.ctc_lo.bias'].clone()
                                #    logging.warning("override src ctc layer from the original one")

                                count = 0
                                for k in partial_state_dict.keys():
                                    if count < len(src_dict):
                                        new_k = k.split('.',1)[0]+'_src.'+k.split('.',1)[1]
                                    else:
                                        new_l = int(k.split('.', 3)[2]) - seg_layer
                                        new_k = k.split('.',1)[0]+'_tgt.'+k.split('.',2)[1] + '.{}.{}'.format(new_l, k.split('.',3)[-1])
                                    count += 1
                                    assert(main_state_dict[new_k].shape == partial_state_dict[k].shape)
                                    main_state_dict[new_k] = partial_state_dict[k].clone()
                                    logging.warning("override {} by {}".format(new_k, k))
                                    print("override {} by {}".format(new_k, k))
                            else:
                                logging.warning(
