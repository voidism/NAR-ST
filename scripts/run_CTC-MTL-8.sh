...
nj=10            # numebr of parallel jobs for decoding
debugmode=1
dumpdir=dump    # directory to dump full features
N=0             # number of minibatches to be used (mainly for debugging). "0" uses all minibatches.
verbose=0       # verbose option
resume= #./exp/train_sp.en_lc.rm_pytorch_encoder_st_init_w_asr_correct_multitask_seg8_bpe8k_specaug_kdmt/results/snapshot.ep.11         # Resume the training from snapshot
seed=1          # seed to generate random number
# feature configuration
do_delta=false

preprocess_config=conf/specaug.yaml
train_config=conf/train_encoder_st_correct_multitask_seg8.yaml
decode_config=conf/decode_wo_asr_correct.yaml
...
asr_model=/espnet/egs/fisher_callhome_spanish/asr1b/exp/train_sp.es_lc.rm_pytorch_train_bpe1000/results/model.val5.avg.best
mt_model=
...
# exp tag
tag="encoder_st_init_w_asr_correct_multitask_seg8_bpe8k_specaug_kdmt" # tag for managing experiments.
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
	...
	if [ -f "${expdir}/results/${trans_model}" ]; then
	    echo "Avg model ${expdir}/results/${trans_model} exist! Skip averaging checkpoints."
	else
       	    average_checkpoints_encoder.py \
                ${opt} \
                --backend ${backend} \
